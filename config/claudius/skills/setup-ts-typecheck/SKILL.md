---
name: setup-ts-typecheck
description: Set up TypeScript type checking hook (tsc --noEmit) for a TypeScript project. Use when adding type validation to pre-commit or CI.
...
Add the hook from [pre-commit-hooks.yaml.template](pre-commit-hooks.yaml.template) to `.pre-commit-config.yaml`. Do NOT duplicate if a `tsc-noEmit` hook already exists.
