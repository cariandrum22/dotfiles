---
name: setup-rails-db-safety
description: Set up database safety tools (strong_migrations, database_consistency) for a Rails project. Use when adding migration and model-DB consistency checks.
...
Create the file using [strong_migrations.rb.template](strong_migrations.rb.template). Set `target_postgresql_version` to match the project's PostgreSQL version.
