---
name: setup-tofu-project
description: One-command setup for a new OpenTofu IaC project with full linting, security scanning, CI, and commit message enforcement. Orchestrates multiple setup skills.
...
Apply each skill in the following order. For each step, follow the instructions defined in that skill's SKILL.md. If a skill's prerequisites are already satisfied (e.g. hooks already exist), skip the redundant parts but still verify correctness.
