---
name: setup-rails-api
description: One-command setup for a Rails API project with full linting, type checking, security scanning, DB safety, test coverage, and CI. Orchestrates multiple setup skills.
...
Apply each skill in the following order. For each step, follow the instructions defined in that skill's SKILL.md. If a skill's prerequisites are already satisfied (e.g. gems already exist), skip the redundant parts but still verify correctness.
