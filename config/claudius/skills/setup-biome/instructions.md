...
Create `biome.json` in the project root from [biome.json.template](assets/biome.json.template). Replace `__BIOME_VERSION__` with the installed Biome version (e.g. `2.3.14`). Determine the version from:
...
Add the hook from [pre-commit-hooks.yaml.template](assets/pre-commit-hooks.yaml.template) to `.pre-commit-config.yaml`. Do NOT duplicate if a `biome-ci` hook already exists.
