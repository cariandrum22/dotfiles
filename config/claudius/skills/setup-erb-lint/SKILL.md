---
name: setup-erb-lint
description: Set up ERB template linting with erb_lint, better_html, and htmlbeautifier for a Rails project. Use when adding view/template quality checks.
...
Create the file in the Rails root using [erb-lint.yml.template](erb-lint.yml.template).

### 3. Create `config/initializers/better_html.rb`

Create the file using [better_html.rb.template](better_html.rb.template). This enforces safe HTML parsing rules that prevent XSS in ERB templates.
