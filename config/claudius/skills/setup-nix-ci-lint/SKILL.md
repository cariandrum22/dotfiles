...
description: Generate a GitHub Actions lint CI workflow from the project's flake.nix hook configuration, using Determinate Systems Nix actions. Use when setting up lint CI for a Nix project with pre-commit hooks.
...
Create the workflow using [lint.yml.template](lint.yml.template) as the base structure. The base provides:
...
If commitlint hooks are present in the flake, append the commit message linting steps. These steps are defined by the `/setup-commitlint` skill's CI template — read `~/.claude/skills/setup-commitlint/ci-steps.yml.template` and append its contents after the "Run pre-commit hooks" step.
