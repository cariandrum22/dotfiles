...
Add the hooks from [pre-commit-hooks.yaml.template](assets/pre-commit-hooks.yaml.template) to the `repos` list. Do NOT duplicate hooks that already exist — if hooks with IDs `just-fmt-check` or `just-evaluate` already exist, skip them.
