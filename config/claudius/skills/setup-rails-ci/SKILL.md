---
name: setup-rails-ci
description: Generate a GitHub Actions CI workflow for a Rails project by reading the Gemfile to determine which tools are installed. Use when setting up Rails-specific CI.
...
Create `.github/workflows/ci.yml` using [ci.yml.template](ci.yml.template) as the base. The template has two jobs:
